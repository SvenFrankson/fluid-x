...
class CarillionMaterials {
    constructor(game) {
        ...
        noboriRedMaterial.diffuseTexture = new BABYLON.Texture("./datas/textures/nobori-red.png");
        noboriRedMaterial.specularColor.copyFromFloats(0, 0, 0);
        noboriRedMaterial.emissiveColor.copyFromFloats(0.3, 0.3, 0.3);
        noboriRedMaterial.freeze();
        let noboriYellowMaterial = new BABYLON.StandardMaterial("nobori-yellow-material");
        noboriYellowMaterial.diffuseTexture = new BABYLON.Texture("./datas/textures/nobori-yellow.png");
        noboriYellowMaterial.specularColor.copyFromFloats(0, 0, 0);
        noboriYellowMaterial.emissiveColor.copyFromFloats(0.3, 0.3, 0.3);
        noboriYellowMaterial.freeze();
        let noboriBlueMaterial = new BABYLON.StandardMaterial("nobori-blue-material");
        noboriBlueMaterial.diffuseTexture = new BABYLON.Texture("./datas/textures/nobori-blue.png");
        noboriBlueMaterial.specularColor.copyFromFloats(0, 0, 0);
        noboriBlueMaterial.emissiveColor.copyFromFloats(0.3, 0.3, 0.3);
        noboriBlueMaterial.freeze();
        let noboriGreenMaterial = new BABYLON.StandardMaterial("nobori-blue-material");
        noboriGreenMaterial.diffuseTexture = new BABYLON.Texture("./datas/textures/nobori-green.png");
        noboriGreenMaterial.specularColor.copyFromFloats(0, 0, 0);
        noboriGreenMaterial.emissiveColor.copyFromFloats(0.3, 0.3, 0.3);
        noboriGreenMaterial.freeze();
        this.noboriMaterials = [];
        this.noboriMaterials[TileColor.North] = noboriRedMaterial;
        this.noboriMaterials[TileColor.East] = noboriYellowMaterial;
        this.noboriMaterials[TileColor.South] = noboriBlueMaterial;
        this.noboriMaterials[TileColor.West] = noboriGreenMaterial;
        /*
    ...
    static HaikuTileUpdateStep(puzzle) {
        if (puzzle.data.id === 74 ||
            ...
            if (puzzle.puzzleState === PuzzleState.Playing) {
                ...
                if (!targetTile) {
                    let buttonValue = 0;
                    for (let v = 1; v <= 3 && buttonValue === 0; v++) {
...
class HaikuTile extends BABYLON.Mesh {
    ...
    async hide(duration = 1) {
        this.shown = false;
        this.tile.outlineColor.copyFromFloats(0, 0, 0);
        return this.animateVisibility(0, duration, Nabu.Easing.easeInOutSine);
    }
    update(dt) {
        this._timer += dt;
        let s = 1 + Math.sin(4 * this._timer) * 0.06;
        this.scaling.copyFromFloats(s, s, s);
        if (this.shown) {
            let c = (1 - Math.sin(4 * this._timer)) * 0.5;
            c = c * c * c;
            this.tile.outlineColor.copyFromFloats(c, c, c);
        }
        else {
            this.tile.outlineColor.copyFromFloats(0, 0, 0);
        }
